...
PR_SIGNATURE = (
    'Automatic fix by [Ghost Ship AI](https://github.com/ghost-ship-ai) — no crew, no captain, the ship sails on.'
)
...
def apply_patch(repo_dir: str, patch: str) -> None:
    ...
    for diff in diffs:
        ...
        new_content = apply_diff(diff, split_content, use_patch=False)
...
def make_commit(
    ...
    issue: Issue,
    issue_type: str,
    git_user_name: str = 'ghost-ship-ai[bot]',
    git_user_email: str = 'ghost-ship-ai[bot]@users.noreply.github.com',
) -> None:
...
def send_pull_request(
    ...
    base_branch_name = f'fix/{issue.number}'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'Ghost Ship AI made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = (
                'A new Ghost Ship update is available, but failed to parse or summarize '
                f'the changes:\n{additional_message}'
            )
...
def main() -> None:
    ...
    parser.add_argument(
        '--git-user-name',
        type=str,
        default='ghost-ship-ai[bot]',
        help='Git user name for commits',
    )
    parser.add_argument(
        '--git-user-email',
        type=str,
        default='ghost-ship-ai[bot]@users.noreply.github.com',
